...
let min = (...nums) => {
    let min = nums[0];
    for (let i of nums) if (i < min) min = i;
    return min;
};
console.log("min(1, -2, 0):", min(1, -2, 0)); // -2

// 8. max()
let max = (...nums) => {
    let max = nums[0];
    for (let i of nums) if (i > max) max = i;
    return max;
};
console.log("max(1, -2, 0):", max(1, -2, 0)); // 1

// 9. pow()
let pow = (a, b) => a ** b;
console.log("pow(2, 5):", pow(2, 5));         // 32
